...
// Progress listeners
const uploadTypes = ["icon", "stable", "experimental"];
    ...
    uploadTypes.forEach((type) => {
        ...
        form.addEventListener("htmx:xhr:loadend", function () {
            ...
            setTimeout(function () {
                ...
                document.getElementById("storageStatusRefreshButton").click();
            }, 1000);
        });
        form.addEventListener("htmx:xhr:progress", (event) => {
            ...
            let bar = progress.children.namedItem("bar");
            bar.firstElementChild.style.width = progressPercent;
            let progressText;
            if (total < 1000000) {
